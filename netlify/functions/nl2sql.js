...
Write a concise, operational plan a shift lead could act on. Structure:
1. One-sentence framing of the hot spot (place + total sites + total customers impacted + how many sites need a field dispatch).
2. Then, for each phase IN THE ORDER GIVEN, a short paragraph or tight bullets:
   - what the work is (in plain NOC terms, informed by the fault classes present),
   - the LABOR & RESOURCES it needs (Phase 1 = field crew / truck roll with tower climb + tilt/power gear; Phases 2-3 = remote NOC engineer, config/parameter changes, no dispatch),
   - the NET BENEFIT stated as the customer-impact number for that phase,
   - if a phase is flagged "recalibrate after Phase 1", note that these remote fixes must wait until the coverage geometry is re-measured post-dispatch.
3. One closing line on sequencing logic: batch the single field dispatch first (it fixes many sites' geometry in one trip), then let remote tuning cascade — that is what makes a dense hot spot a cheap, high-impact win.

Rules:
- Use the EXACT customer numbers and site counts provided. Never invent new ones.
- Keep it tight and skimmable. No preamble like "Certainly". No markdown headers with '#'. Short bold labels and bullets are fine.
- Measure benefit in CUSTOMERS, matching the tool.`;
